...
void receiveAndRespond(int socketFd, const string& name) {
    ...
    while (true) {
        ...
        // Update rolling price history (keep last 3)
        if (priceHistory.size() >= 3)
            priceHistory.pop_front();
        priceHistory.push_back(price);

        // Need at least 3 prices to detect momentum
        if (priceHistory.size() < 3) {
            cout << "Building history (" << priceHistory.size() << "/3). Skipping price ID " << priceId << endl;
            ++missCount;
            continue;
        }
        ...
        if (up || down) {
            ...
            cout << "Momentum " << direction << " detected [" << a << " -> " << b << " -> " << c << "]. "
                 << "Sending order for price ID " << priceId << endl;

            // Fast reaction delay (10–60ms) to compete for the hit
            ...
            ++hitCount;
            cout << "Order sent for price ID " << priceId
                 << " | Hits: " << hitCount << ", Passes: " << missCount << endl;
        } else {
            cout << "No momentum [" << a << " -> " << b << " -> " << c << "]. "
                 << "Ignoring price ID " << priceId << endl;
            ++missCount;
        }
    }

    cout << "Session summary - Hits: " << hitCount << ", Passes: " << missCount << endl;
    close(socketFd);
}
